...
export function createUnityInstance(
	...
	function s(e, t) {
		if (!s.aborted && config.showBanner)
			return (
				"error" == t && (s.aborted = !0),
				config.showBanner(e, t)
			);
		switch (t) {
			case "error":
				console.error(e);
	...
	function r(e) {
		...
		(n +=
			...
			D(
				n,
				e.filename ||
					(t && (t.fileName || t.sourceURL)) ||
					"",
				e.lineno ||
					(t && (t.lineNumber || t.line)) ||
					0
			);
	}
	...
	for (o in (configValidator(
		config,
		"companyName",
		"Unity"
	),
	configValidator(config, "productName", "WebGL Player"),
	configValidator(config, "productVersion", "1.0"),
	config))
		l[o] = config[o];
	...
	var u = "",
		d = "";
	function h(e) {
		document.webkitCurrentFullScreenElement === canvas
			? canvas.style.width &&
				((u = canvas.style.width),
				(d = canvas.style.height),
				(canvas.style.width = "100%"),
				(canvas.style.height = "100%"))
			: u &&
				((canvas.style.width = u),
	...
	function x() {
		var e = this;
		(this.isConnected = this.connect().then(
			function () {
	...
	function U(e) {
		return (
			(U.link =
				U.link || document.createElement("a")),
			(U.link.href = e),
			U.link.href
		);
	}
	...
	return (
		(l.SystemInfo = (function () {
			...
			return {
				...
				mobile: /Mobile|Android|iP(ad|hone)/.test(
		...
		(l.UnityCache =
			...
			(x.prototype.loadRequestMetaData = function (
				e
			) {
				e = "string" == typeof e ? e : e.url;
				return this.execute(p.name, "get", [e]);
			}),
			(x.prototype.updateRequestMetaData = function (
				e
			) {
			...
			(x.prototype.upgradeDatabase = function (e) {
				...
				e.objectStoreNames.contains(p.name) ||
					((t = e.createObjectStore(p.name, {
						keyPath: "url"
					})),
					["accessedAt", "updatedAt"].forEach(
						function (e) {
							t.createIndex(e, e);
						}
					)),
					e.objectStoreNames.contains(m) &&
						e.deleteObjectStore(m),
			...
			(x.prototype.cleanUpCache = function () {
				var i = this;
				return this.getMetaDataEntries().then(
					function (e) {
						for (
						...
						o.sort(function (e, t) {
							return (
								e.accessedAt - t.accessedAt
							);
						});
						for (
							a = 0;
							a < o.length &&
							!(n < x.MaximumCacheSize);
							++a
						)
