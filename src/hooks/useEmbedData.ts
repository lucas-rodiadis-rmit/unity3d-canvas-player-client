 ...
 */
function submitLTIForm({ contentItems, returnUrl }: {
	contentItems: object;
	returnUrl: string;
  }) {
	...
	addHiddenInput("lti_message_type", "ContentItemSelection");
	addHiddenInput("lti_version", "LTI-1p0");
	addHiddenInput("content_items", JSON.stringify(contentItems));
...
function useEmbedData() {
	const submitEmbedData = (useIFrame: boolean) => {
	  if (!window.LOCAL_DATA.token || !window.LOCAL_DATA.returnUrl) {
		alert("Missing token or return URL. Try refreshing the page.");
		return;
	  }
		...
		"@graph": [
		  {
			...
			"placementAdvice": {
			  presentationDocumentTarget: useIFrame ? "iframe" : "frame",
			  ...(useIFrame && {
				displayWidth: 1067,
				displayHeight: 600
			  }),
